...
double Power_FL, Power_FR, Power_BL, Power_BR;
float yawOffset = 0;
int spd = 100;
bool state_up = false;
...
void setup() {
    ...
    servoWrite(0, 150); //gripper {less = release}
    servoWrite(1, 128); // turn up  {more = down}
    servoWrite(2, 60); //grab up   {more = down}
...
void grab_send() {
  ...
  if (controller.triangle) {
    ...
    if (state_up) {
      ...
      //--- TAKE DOWN CASE ---//
      servoWrite(0, 150); //gripper // RELEASE PAY SAFE
      delay(100);
      servoWrite(1, 5); // send up //SEND UP
      delay(400);
      servoWrite(0, 110); //gripper //GRAB
      delay(100);
      servoWrite(2, 60); //grab up  //RELEASE
      delay(100);
      servoWrite(1, 128); //turn up //RETURN NORMAL STATE
      ...
      servoWrite(1, 5); // send up //SEND UP
      delay(400);
      servoWrite(2, 120); //grab up //LOCK RING
      delay(100);
      servoWrite(0, 150); //gripper //RELEASE GRIPPER
      delay(100);
      servoWrite(1, 128); //turn up //RETURN NORMAL STATE
